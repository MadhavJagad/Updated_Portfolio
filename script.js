...
// Split lines for nav links
const splitLinks = document.querySelectorAll(".nav-items a");
splitLinks.forEach((a) => {
  const text = a.textContent;
  a.innerHTML = `<span class="line">${text}</span>`;
});
...
document.querySelectorAll("#site-footer a[href^='#']").forEach((link) => {
  link.addEventListener("click", (e) => {
    ...
    if (href && href.length > 1) {
      e.preventDefault();
      const target = document.querySelector(href);
      if (target) lenis.scrollTo(target, { duration: 1.5 });
    }
  });
});

// Hero buttons
document.getElementById("heroViewWork").addEventListener("click", () => {
  lenis.scrollTo(document.getElementById("work-section"), { duration: 1.5 });
});
document.getElementById("heroContact").addEventListener("click", () => {
  lenis.scrollTo(document.getElementById("contact-section"), {
    duration: 1.5,
  });
});
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  gsap.set(heroWrap, { clipPath: "polygon(50% 50%,50% 50%,50% 50%,50% 50%)" });
  ...
  loaderTl.add(() => {
    const chars = counterEl.textContent.split("").map((c) => {
    ...
    counterEl.innerHTML = "";
    chars.forEach((c) => counterEl.appendChild(c));
    gsap.to(counterEl.querySelectorAll(".digit"), {
      y: "-120%",
      duration: 0.55,
      ease: "power3.in",
      stagger: 0.06,
...
function initScrollAnimations() {
  ...
  // Contact section left col paragraphs
  gsap.fromTo(
    "#contact-section .sub-txt, #contact-section .info-stack",
    { opacity: 0, x: -20 },
    {
      opacity: 1,
      x: 0,
  ...
  // Highlights masonry cards are handled by the IntersectionObserver below
  ...
  // Section eyebrows - reveal line before text
...
// ─── HIGHLIGHT CARDS INTERSECTION OBSERVER ───
(function () {
...
(function initMatterJS() {
  ...
  ScrollTrigger.create({
    ...
    onEnter: () => {
      ...
      script.onload = () => setupPhysics(container, section);
...
(function () {
  ...
  function drawTrail() {
    ...
    tCtx.strokeStyle = "rgba(0,240,255,.55)";
    tCtx.lineWidth = 1;
    tCtx.shadowColor = "rgba(0,240,255,.4)";
    tCtx.shadowBlur = 12;
    tCtx.stroke();
    ...
    tCtx.shadowColor = "rgba(0,240,255,.9)";
    tCtx.shadowBlur = 20;
    tCtx.fill();
    tCtx.shadowBlur = 0;
  }
...
(function () {
  ...
  for (let t = 0; t <= 1; t += 0.02) {
    ...
    const x = -0.55 + t * 0.3,
      y = 0.6 - t * 1.5 + Math.sin(t * Math.PI) * 0.3,
      z = Math.cos(angle) * 0.15 * (1 - t * 0.3),
      w = Math.sin(angle) * 0.15 * (1 - t * 0.3);
